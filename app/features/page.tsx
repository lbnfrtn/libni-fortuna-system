...
export const metadata = {
  title: "Featured — TV & media, guest podcasts, publications & stages · Libni Fortuna",
  description: "Where Libni Fortuna has been featured: television and media, guest podcast appearances, national publications, and the stages and events she has led.",
};

const TABS: [string, string][] = [["#television", "TV & media"], ["#podcasts", "Guest podcasts"], ["#publications", "Publications"], ["#events", "Speaking & stages"]];
  ...
  return (
    <SitePage>
      <EdHeroSimple
        ...
        lede="Television and media, guest podcast conversations, national publications — and the stages and rooms she has led. Every link, in one place."
        ctas={[{ label: "Invite me to speak", href: "/speaking", variant: "ink" }, { label: "Media kit", href: "/media-kit", variant: "ghost" }]}
      />
      ...
      <section className="ed-sec-sm ed-ivory">
        <div className="ed-wrap">
          <EdStats items={[[String(television.length), "TV & media"], [String(podcasts.length), "Guest podcasts"], [String(publications.length), "Publications"], [String(events.length), "Events & stages"]]} />
        </div>
      </section>

      {/* 01 · TV & MEDIA */}
      <section className="ed-sec ed-ivory" id="television" style={{ paddingTop: "clamp(24px, 4vw, 56px)" }}>
        <div className="ed-wrap">
          <div className="ed-head ed-reveal">
            <div><p className="ed-eyebrow">01 · TV &amp; media</p><h2 className="ed-display">On air.</h2></div>
      ...
      {/* 02 · GUEST PODCAST APPEARANCES */}
      <section className="ed-sec ed-night" id="podcasts">
        <div className="ed-wrap">
          <div className="ed-head ed-reveal">
            <div><p className="ed-eyebrow">02 · Guest podcast appearances</p><h2 className="ed-display">As a guest.</h2></div>
            <div className="ed-stack-sm">
              ...
              <p style={{ color: "rgba(251,249,246,.6)", fontSize: 15 }}>Looking for her own show? That’s <Link href="/podcast" style={{ borderBottom: "1px solid var(--ed-gold)" }}>Anyway, Moving Forward</Link>{podcast.items.length ? ` — ${podcast.items.length} episodes` : ""}.</p>
            </div>
          </div>
          {podcasts.length > 0
            ? <div className="ed-press ed-reveal">{podcasts.map((p) => <PressCard key={p.id} p={p} photos={content.photos} prev={prev} />)}</div>
            : <p className="ed-lede" style={{ color: "rgba(251,249,246,.6)" }}>Guest appearances are added from the Studio.</p>}
        </div>
      </section>
      ...
      <section className="ed-sec ed-ivory" id="events" style={{ paddingTop: 0 }}>
        <div className="ed-wrap">
          <div className="ed-head ed-reveal">
            <div><p className="ed-eyebrow">04 · Speaking &amp; stages</p><h2 className="ed-display">The most recent rooms.</h2></div>
            <div className="ed-stack-sm">
              <p className="ed-lede ed-muted">Keynotes, workshops, panels, retreats and facilitations — {events.length} and counting.</p>
              <p><Link href="/speaking" className="ed-link">Book Libni to speak</Link></p>
            </div>
          </div>
      ...
      <section className="ed-sec-sm ed-night">
        <div className="ed-wrap ed-split">
          ...
          <div className="ed-off1 ed-reveal">
            <EdCtas ctas={[{ label: "Invite me to speak", href: "/speaking", variant: "gold" }, { label: "Download the media kit", href: "/media-kit", variant: "light" }]} />
      ...
      <EdFinal title="Bring the room home to itself." ctas={[{ label: "Invite me to speak", href: "/speaking", variant: "gold" }, { label: "Hear the podcast", href: "/podcast", variant: "light" }]} />
    </SitePage>
  );
}
